...
namespace upscale_rpc::response::single
{
    ...
    void serialize(const context_id_t context_id, const call_data& call, raw_data_t& buffer)
    {
        upscale_rpc::serialize<header>(call, buffer,
                                       [context_id](std::uint8_t* buffer_data, const std::size_t serialization_buffer_size) noexcept
                                       {
                                           ...
                                           header_item->basic_header = primary_header(action_t::response, encoding_scheme_t::single);
    ...
    {
        auto result = upscale_rpc::deserialize<call_data, header>(buffer, action_t::response, encoding_scheme_t::single);
